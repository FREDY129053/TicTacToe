    ...
    <>
      {rooms.length !== 0 ? (
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
          {rooms.map((room) => (
            <Link href={`/game/${room.uuid}?is_hard=${room.is_difficult}`} key={room.uuid}>
            <div
        ...
        <div className="text-white text-2xl flex items-center justify-center h-full min-h-[calc(100vh-200px)]">Нет игровых комнат, но вы можете создать...</div>
      )}
    </>
  );
}
